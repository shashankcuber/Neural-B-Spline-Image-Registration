...
class SpatialTransformation(nn.Module):
    ...
    def meshgrid(self, height, width):
        ...
        if self.use_gpu==True:
            x_t = x_t.to(device='cuda:1')
            y_t = y_t.to(device='cuda:1')
...
class VoxelMorph2d(nn.Module):
    def __init__(self, in_channels, use_gpu=False):
        ...
        if use_gpu:
            self.unet = self.unet.to(device='cuda:1')
...
def cross_correlation_loss(I, J, n):
    ...
    if use_gpu:
        sum_filter = sum_filter.to(device='cuda:1')
...
def visualise_results(fixed, moving, pred, xy_, xyd, epoch):
    """

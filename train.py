...
def main():
    ...
    # filename = list(set([x.split('_')[0] for x in os.listdir('./fire-fundus-image-registration-dataset/')]))
    ...
    with wandb.init(project="B-spine-Image-Registration", config=train_config, name=f"B-spline-Image-Registration-{train_config['num_points']}-num_points") as run:
        ...
        for epoch in range(train_config["epochs"]):
            ...
            for batch_fixed, batch_moving in validation_generator:
                loss, dice = rnet.get_test_loss(batch_moving, batch_fixed, epoch, count)
                count += 1
